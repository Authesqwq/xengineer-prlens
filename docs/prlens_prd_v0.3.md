...
| v0.3 | Authes | 2026-05-30 | 根据 PR1-PR17 实现结果更新 MVP 范围，补充分析模式、侧栏工作区、会话历史、导出报告、耗时统计、进度反馈和风险 fallback；将示例 PR 与反馈入口调整为后续规划 |
...
| 8 | P1 | 分析模式 | 支持快速、标准、完整三种模式，分别控制变更总结、风险分析和 Review 建议的执行范围 |
| 9 | P1 | 分析进度与耗时 | 展示阶段状态、完成状态和分析耗时，降低用户误判卡住的概率 |
| 10 | P1 | 会话历史记录 | 使用本次会话历史保存最近分析结果，支持点击恢复，不重新请求 GitHub API 和 LLM |
| 11 | P1 | Markdown 报告导出 | 支持导出当前分析结果，包含 PR 概览、总结、风险、建议、模式和耗时 |
| 12 | P1 | 中英文双语 | 支持中文 / English UI，并通过 prompt 控制模型自然语言输出 |
| 13 | P1 | 风险分析 fallback | 风险分析空响应或非法 JSON 时，不暴露解析错误，返回结构化兜底结果与限制说明 |
| 14 | P2 | 结果反馈入口 | 后续收集有用、误报、依据不足等反馈，用于评测集和 bad case 回流 |
| 15 | P2 | 示例 PR 与 Demo 案例 | 不在主页面提供示例入口，改由 README、Demo 视频和评测案例文档承载 |
| 16 | P2 | 多平台支持 | 后续扩展 GitLab、Gitee 等平台 |
...
| 模型分析 | 生成总结、风险和建议 | PRInfo、ModelContext、分析模式 | Summary、RiskResult、ReviewSuggestions | P0 |
| 分析模式 | 控制执行范围 | 快速 / 标准 / 完整 | 对应的 Summary、Risk、Suggestions 执行链路 | P1 |
| 进度与耗时 | 展示真实阶段状态和耗时 | 分析流程状态 | 阶段进度、完成状态、elapsed time | P1 |
| 结果展示 | 卡片化展示分析结果 | 分析结果 | 页面结果区 | P0 |
| 侧栏工作区 | 管理语言、模式、历史和导出 | 用户设置、历史记录 | 工作区侧栏 | P1 |
| 会话历史 | 保存最近分析结果并支持恢复 | 当前分析结果 | 最近 10 条历史记录 | P1 |
| 报告导出 | 导出 Markdown 分析报告 | 当前展示结果 | `.md` 报告文件 | P1 |
| 用户反馈 | 收集正负反馈 | 风险项、建议项 | FeedbackEvent | P2 |
| Demo 案例 | 固定评测与演示样例 | README / evaluation case | 可复现演示材料 | P2 |
...
当前 MVP 以本地 Demo 和实训评审为目标，不接入数据库和线上埋点。已实现的数据记录主要保存在本次 Streamlit session 中，用于提升单次使用体验；长期数据回流作为后续公开部署或团队试用阶段规划。

| 数据类型 | 当前阶段 | 采集内容 | 用途 |
|---|---|---|---|
| 会话历史 | 已实现 | PR URL、owner/repo、PR number、标题、语言、分析模式、风险等级、耗时、分析结果 | 支持最近分析结果恢复与 Demo 演示 |
| 分析耗时 | 已实现 | 单次分析 elapsed time | 展示性能反馈，辅助判断不同模式耗时 |
| 分析模式 | 已实现 | 快速、标准、完整 | 控制 Summary、Risk、Review Suggestions 执行范围 |
| diff 截断状态 | 已实现 | 文件总数、纳入数、跳过数、截断数、字符数变化 | 向用户说明模型输入范围 |
| 执行链路埋点 | 后续规划 | `pr_url_submitted`、`github_fetch_succeeded`、`diff_truncated`、`llm_started`、`analysis_failed` | 定位流程瓶颈和失败环节 |
| 用户操作埋点 | 后续规划 | 复制建议、导出报告、历史恢复、模式切换、反馈点击 | 判断功能是否被实际使用 |
| 正向 case | 后续规划 | 用户点击“有用”、复制建议、无修改采纳 Review 建议 | 建立高质量样本 |
| 负向 case | 后续规划 | 用户点击“误报”、反馈“依据不足”、重新分析后结果修正 | 建立 bad case 集 |
| 业务数据 | 后续规划 | 分析完成率、平均耗时、建议复制率、风险误报率、JSON 解析失败率 | 验收功能价值 |
| 模型数据 | 后续规划 | prompt 版本、模型名称、输入长度、输出长度、截断状态 | 支持 prompt 回归评测和成本监控 |
...
```mermaid
flowchart TD
    A[用户打开 PRLens 页面] --> B[在侧栏选择语言和分析模式]
    B --> C[输入 GitHub PR 链接]
    C --> D[点击开始分析]
    D --> E{链接是否合法}
    E -- 否 --> E1[提示正确链接格式]
    ...
    I --> J{分析模式}
    J -- 快速模式 --> K[生成变更总结]
    J -- 标准模式 --> L[生成变更总结和风险分析]
    J -- 完整模式 --> M[生成变更总结、风险分析和 Review 建议]
    K --> N[展示结果并写入会话历史]
    L --> N
    M --> N
    N --> O[用户查看结果、复制建议或导出报告]
    O --> P[可从历史记录恢复结果]
```

用户视角的关键体验要求：

1. 主页面只保留一个 PR 链接输入框和一个开始分析入口，避免示例区干扰主流程。
2. 语言、分析模式、历史记录、导出报告和清空历史统一放在侧栏工作区。
3. 分析过程中展示阶段状态，例如“正在获取 PR 信息”“正在整理 diff”“正在生成 Review 建议”。
4. 分析完成后展示耗时，并将结果写入最近 10 条会话历史。
5. 输出结果优先展示高风险项，再展示中低风险项。
6. 对模型不确定的结果显示“需要人工确认”。
7. 每条建议提供可复制评论文本；用户反馈入口作为后续迭代规划。
...
flowchart TD
    ...
    H1 --> I{判断/路由节点：分析模式}
    H2 --> I
    I -- 快速模式 --> J[LLM 调用节点：PR 总结生成]
    I -- 标准模式 --> K[LLM 调用节点：PR 总结生成]
    I -- 完整模式 --> L[LLM 调用节点：PR 总结生成]
    K --> M[LLM 调用节点：风险识别]
    L --> M
    M --> N{判断/路由节点：风险 JSON 是否可解析}
    N -- 否 --> N1[异常处理：重试一次；仍失败则返回结构化 fallback]
    N -- 是 --> O[工具调用节点：风险结果校验、排序、格式化]
    N1 --> O
    O --> P{完整模式且存在具体风险项}
    P -- 是 --> Q[LLM 调用节点：Review 建议生成]
    P -- 否 --> R[跳过 Review 建议，展示原因]
    J --> S[页面展示：总结、进度、耗时]
    Q --> T[页面展示：总结、风险、建议、进度、耗时]
    R --> U[页面展示：总结、风险、跳过说明、进度、耗时]
    S --> V[会话历史：写入当前结果]
    T --> V
    U --> V
    V --> W[用户查看、复制建议或导出 Markdown 报告]
```
...
| LLM 调用节点 | 风险识别 | 输入 summary、diff 和风险分类说明，输出 risks、limitations 等结构化结果 |
| LLM 调用节点 | Review 建议生成 | 仅在完整模式且存在具体风险项时执行，输出可复制建议 |
| 工具调用节点 | GitHub REST API | 获取 PR 基本信息和 changed files |
| 工具调用节点 | diff_processor | 清洗 diff、控制长度、生成截断说明 |
| 判断/路由节点 | URL 校验、PR 可访问、上下文限制、分析模式、JSON 校验 | 决定后续链路和异常分支 |
| 异常处理分支 | API 失败、rate limit、patch 为空、模型超时、JSON 解析失败 | 页面显示明确错误或结构化 fallback，不暴露 traceback 和 API Key |
| 会话历史节点 | 历史记录 | 保存最近 10 条分析结果，支持点击恢复，不重新请求 GitHub 或 LLM |
...
| 约束维度 | 要求 | 说明 |
|---|---|---|
| 代码理解能力 | 能理解常见 Web/Python/JS/TS diff | MVP Demo 验证 PR 优先选择通用开源项目和前后端代码 |
...
| Demo 前回归 | 40 | 增加 2-3 个Demo 验证 PR 的完整链路 |
...
5. 对 Demo 验证 PR 建立固定快照，避免公开 PR 后续变更影响 Demo。
...
| 策略 | 配置 | 说明 |
|---|---|---|
| GitHub API timeout | 10 秒 | 超时后展示错误并允许用户重新分析 |
| LLM timeout | 60 秒 | 超时后保留 PR 基本信息和 diff 统计 |
| 请求重试 | 1-2 次 | 仅对网络失败和 5xx 生效 |
| Rate limit 提示 | 检查状态码和响应头 | 提示配置 GitHub Token 或稍后重试 |
| diff 截断 | 单文件与总字符数双阈值 | 防止输入过长导致失败，并展示字符数变化 |
| 风险分析 JSON fallback | 重试一次；仍失败则降级 | 不直接暴露 JSONDecodeError，返回结构化低风险兜底结果和限制说明 |
| 模型空响应处理 | 返回 fallback | 提示模型未返回可解析风险项，并要求人工审查 |
| Review 建议跳过 | 无具体风险项时跳过 | 避免在无依据情况下生成建议 |
| 会话历史 | 最近 10 条 | 相同 PR、语言和模式去重，点击恢复不重新请求 API 和 LLM |
| 降级展示 | 模型失败时展示 PR 概览和文件变更 | 保证页面仍有信息价值 |
| Demo 验证 PR | README 与评测案例中维护 | 降低现场演示失败概率，主页面不提供示例入口 |
...
MVP 采用单页结构。侧栏作为工作区，主页面只承载核心分析流程。

```text
┌──────────────────────────────────────┬──────────────────────────────────────────────┐
│ 侧栏工作区                            │ 主页面                                       │
│ PRLens                                │ PRLens：AI PR Review 助手                    │
│                                      │ 输入 GitHub PR 链接，生成变更总结、风险分析… │
│ 语言                                  │                                              │
│ [中文 / English]                      │ [ GitHub PR URL 输入框 ]                     │
│                                      │ [开始分析]                                   │
│ 分析模式                              │                                              │
│ [快速 / 标准 / 完整]                  │ 分析进度                                     │
│                                      │ ✓ 解析 PR 链接                               │
│ 历史记录                              │ ✓ 获取 PR 信息                               │
│ - owner/repo #123 · 标准 · 18.6 秒    │ ▶ 生成变更总结                               │
│                                      │                                              │
│ 操作                                  │ PR 概览                                      │
│ [导出报告]                            │ 变更文件列表                                 │
│ [清空历史]                            │ Diff 上下文统计                              │
...
1. 分阶段展示 loading 文案和进度状态。
2. 当前阶段显示进行中，已完成阶段显示完成，跳过阶段显示跳过原因。
3. 分析完成后展示耗时。
4. 快速模式不显示风险分析和 Review 建议为等待中；标准模式不显示 Review 建议为等待中。
5. 模型分析失败时给出失败阶段、错误类型和可操作提示。

结果不确定性处理：

1. 低置信度风险用弱提示展示。
2. “需要人工确认”放在风险卡片显眼位置。
3. diff 截断时在结果顶部或 diff 统计中展示分析范围有限。
4. 风险分析无法解析时展示结构化 fallback，不暴露 JSON 解析错误。

用户操作：

1. 每条 Review 建议提供可复制文本。
2. 侧栏提供 Markdown 报告导出。
3. 侧栏历史记录支持恢复结果。
4. 用户反馈入口后续补充，不作为当前 MVP 必选功能。
...
8. 会话历史仅保存在当前 Streamlit session 中，刷新或重启后不保证保留。

### 已知缺陷

| 缺陷 | 当前处理 | 修复计划 |
|---|---|---|
| 跨文件深层依赖理解不足 | 标注低置信度和人工确认 | 后续引入仓库文件检索 |
| 长 diff 可能遗漏后部文件 | 截断并提示分析范围，展示字符数变化 | 后续增加分文件分析与汇总 |
| JSON 输出偶发失败 | 风险分析阶段重试一次，仍失败则返回结构化 fallback | 持续优化 Schema、parser 和 prompt 回归评测 |
| 测试缺失判断可能误报 | 只作为建议展示，不作为合并结论 | 后续引入测试目录识别规则 |
| 会话历史不可持久化 | 仅保存本次 Streamlit session | 后续公开部署时再评估本地缓存或数据库 |
| Demo 验证 PR 后续可能变化 | README 与评测案例记录稳定样例 | Demo 前固定样例并准备录屏备份 |

---

## 十一、数据飞轮与迭代机制

当前 MVP 不接入数据库和线上埋点，数据飞轮作为后续公开部署或团队试用阶段的迭代机制。已实现的会话历史、分析耗时和导出报告主要服务于 Demo 体验，不等同于长期数据回流。

### 11.1 用户反馈采集

| 反馈类型 | 采集方式 | 当前阶段 | 说明 |
|---|---|---|---|
| 显式反馈：风险有用 | 风险卡片点击“有用” | 后续规划 | 进入正向样本 |
| 显式反馈：误报 | 风险卡片点击“误报” | 后续规划 | 进入 bad case |
| 显式反馈：依据不足 | 风险卡片点击“依据不足” | 后续规划 | 优先检查 evidence prompt |
| 显式反馈：建议可用 | 点击“复制建议” | 后续规划 | 作为可用性正向信号 |
| 隐式反馈：重复分析 | 同一 PR 多次 Analyze | 可由会话历史间接观察 | 可能表示结果不稳定 |
| 隐式反馈：分析中断 | 用户在分析完成前离开 | 后续规划 | 可能表示耗时过长 |
| 人工标注 | PM 对 bad case 标注失败原因 | 后续规划 | 用于评测集维护 |
...
|---|---|:---|---|---|
| 本地自测 | 1 人 | PR1-PR10 | 主流程跑通、错误处理可用 | 3 个 Demo 验证 PR 均可完成分析 |
...
|---|---|---|---|---|
| 数据获取风险 | GitHub API rate limit 或 PR 不可访问 | 中 | 中 | 支持 Token、错误提示、Demo 验证 PR |
...
| Demo 风险 | Demo 验证 PR 变化或模型不稳定 | 中 | 高 | 维护多个 Demo 案例、Demo 前录制验证 |
